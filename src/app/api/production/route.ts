...
export async function POST(req: Request) {
  ...
  const cavities = parseFloat(d.cavities) || 1
  ...
  if (d.mould && shotsThisShift > 0) {
    ...
    if (mould && mould.pm_frequency_shots > 0) {
      ...
      await supabase
        .from('mould_master')
        .update({ current_shots: newShots, status: newStatus })
        .eq('id', mould.id)
    }
  }
  ...
  return NextResponse.json({
    success: true,
    msg: `Production saved! Good: ${Math.round(totalGood)} | Rej: ${Math.round(totalRej)} | Shots: ${shotsThisShift}`
  })
}
